...
echo '6.'
cat data-hold/legislators-current.json | jq '.[] .bio .religion' | sort | uniq -c | sort -nr | tr -d '"'
echo '7.'
cat data-hold/legislators-current.json | jq --raw-output '.[] .terms[0] .start' | sort | cut -d '-' -f 1 | grep -c '2015'
echo '8.'
cat data-hold/legislators-current.json | jq '.[] .terms[0] .start' | cut -d '-' -f 1| sort | head -n 1 | tr -d '"'
echo '9.'
cat data-hold/legislators-current.json | jq --raw-output '.[] .terms[-1:][0].party' | grep -c 'Republican'
echo '10.'
cat data-hold/legislators-current.json | jq --raw-output '.[] .terms[-1:][0].party' | grep -Ev -c 'Republican|Democrat'
echo '11.'
...
echo '18.'
cat data-hold/congress-twitter-profiles.json | jq --raw-output '.[] | [.name, .screen_name, .followers_counecho '1.'
cat data-hold/legislators-current.json | jq '.[0] .name .last'
echo '2.'
cat data-hold/legislators-current.json | jq '.[111] .id .bioguide'
echo '3.'
